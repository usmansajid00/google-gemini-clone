...
const ContextProvider = (props) => {
  ...
  useEffect(() => {
    ...
    const storedPreviousPrompt = JSON.parse(
      localStorage.getItem("previousPrompt")
    );
    if (storedInput) setInput(storedInput);
    if (storedRecentPrompt) setRecentPrompt(storedRecentPrompt);
    if (storedPreviousPrompt) setPreviousPrompt(storedPreviousPrompt);
  }, []);
  ...
  }, [input, recentPrompt, previousPrompt]);
  ...
  const onSent = async (prompt) => {
    ...
    for (let i = 0; i < responseArray.length; i++) {
      ...
      for (i = 0; i < newResponseArray.length; i++) {
